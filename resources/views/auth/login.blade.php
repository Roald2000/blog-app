...
@section('content')
    <section class="max-w-lg border-2 shadow-md mt-6 mx-auto p-3 rounded">
        ...
        <form class="w-full" action="{{ route('auth.login-user') }}" method="POST">
            @csrf
            <div class="w-full">
                <label for="email" class="flex flex-col gap-1">
                    <span>Email</span>
                    <input class="p-2 rounded border-2" type="email" name="email" id="email" value="{{old('email')}}" autofocus>
                </label>
            ...
            <br>
            <button class="p-2 rounded bg-blue-600 text-white">Login</button>
        </form>
    </section>
